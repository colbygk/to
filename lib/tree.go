...
/*
func newLeaf(depth int, state []byte, content []byte, lineNo int) (leaf *ToTree) {
  leaf = new(ToTree)
  leaf.Depth = depth
  leaf.State = state[:]
...
func addBranch(parent, branch *ToTree, trackingFile *ToFile, startLine int) (i int) {
  ...
  // Only should happen at the start of root
  if branch == nil {
    branch = parent
  ...
  for i < noItems {
    ...
    // Deconstruct the individual line
    ...
    if UnknownState(state) && Symbol(state[0]) != Continue {
      ...
      // it starts with a character that is not

      // Gaurd against continuations on the first line:
      if i == 0 {
        fmt.Printf("Error - Continuation on first line, not allowed")
        os.Exit(-1)
      }
      ...
      if depth > branch.Depth {
        fmt.Printf(" ADDING BRANCH - %q\n", content)
        leaf = newLeaf(depth, state, content, i)
        i = addBranch(branch, leaf, trackingFile, i+1)
      }
      // Sibling
      if depth == branch.Depth {
        fmt.Printf(" ADDING LEAF - %q\n", content)
        leaf = newLeaf(depth, state, content, i)
